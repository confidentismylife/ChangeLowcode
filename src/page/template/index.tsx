  ...
  useEffect(() => {
    if (id && objectTotal) {
      ...
      if (selected && selected.componentForm) {
        // 过滤出 isTemplate 为 true 的组件
        const templateComponents = selected.componentForm.filter(component => component.isTemplate);
        
        setSelectedComponent(templateComponents);

        const updatedItems3 = templateComponents.map((item, index) => ({
          key: `${item.form}`, 
